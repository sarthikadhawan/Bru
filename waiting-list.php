...
<div class=" container    container-fixed-lg">
    ...
    <tbody>
                                     <?php
                                    $app_id = $_GET['id'];
                                    $name="SELECT a.priority, a.distance, a.request_date, b.name, b.roll_no FROM `request_table` a INNER JOIN `student_profile` b ON a.user_id = b.user_id WHERE a.status = 0 AND a.application_id = $app_id ORDER BY a.priority ASC";
                                    ...
                                    $received_requests_row_count=mysqli_num_rows($received_requests_result2);
                                       ...
                                       if($received_requests_row_count>0)
                                       {
                                		while ( $row2 = mysqli_fetch_assoc($received_requests_result2))
                                		{
                                				...
                                				$date=$row2['priority'];
                                    	    	$time = $row2['name'];
                                    	    	$category=$row2['roll_no'];
                                    	    	$feedback = $row2['request_date'];
